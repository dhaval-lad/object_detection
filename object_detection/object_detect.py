...
class MinimalPublisher(Node):
    ...
    def timer_callback(self):
        #try:
        #    trans = self.tf_buffer.lookup_transform('base_link', 'imu_link', rclpy.time.Time())
        ...
        for result in results.xyxy[0]:
            ...
            if class_name == "person" or class_name =='bicycle':
                if confidence > confidence_threshold:
                    ...
                    y_imu=x_center
                    x_imu=object_depth+0.34
                    x_odom=x_imu+xtf
                    y_odom=-(y_imu+ytf)
                    # scaling_factor_image_to_imu = 1.0 / (object_image_frame + camera_displacement)
                    # image_to_imu=x_center*scaling_factor_image_to_imu
                    label = f"({float(x_center):.3f}): {object_depth:.2f}m"


                    # Publish x coordinates and object depth
                    msg.data = str(float(x_center))+'+'+ str(float(object_depth))
